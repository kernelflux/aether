...
#include <functional>  // For std::bind (C++11 standard, replaces std::bind)
...
#include "scope_recursion_limit.h"
#include "bootrun.h"
#include "tickcount.h"
...
#ifdef __APPLE__
// iOS specific - not needed for Android
// #include "objc/data_protect_attr.h"
#endif
...
void appender_open(TAppenderMode _mode, const char* _dir, const char* _nameprefix, const char* _pub_key, bool _is_compress) {
    assert(_dir);
    assert(_nameprefix);
    ...
    if (OpenMmapFile(mmap_file_path, kBufferBlockLength, sg_mmmap_file))  {
        sg_log_buff = new LogBuffer(sg_mmmap_file.data(), kBufferBlockLength, _is_compress, _pub_key);
        use_mmap = true;
    } else {
        char* buffer = new char[kBufferBlockLength];
        sg_log_buff = new LogBuffer(buffer, kBufferBlockLength, _is_compress, _pub_key);
        use_mmap = false;
    }
    ...
    xlogger_appender(NULL, "AETHER_PATH: " AETHER_PATH);
    xlogger_appender(NULL, "AETHER_REVISION: " AETHER_REVISION);
    xlogger_appender(NULL, "AETHER_BUILD_TIME: " AETHER_BUILD_TIME);
    // Only output AETHER_URL and AETHER_BUILD_JOB if they have values
    if (strlen(AETHER_URL) > 0) {
        xlogger_appender(NULL, "AETHER_URL: " AETHER_URL);
    }
    if (strlen(AETHER_TAG) > 0) {
        xlogger_appender(NULL, "AETHER_BUILD_JOB: " AETHER_TAG);
    }
...
void appender_open_with_cache(TAppenderMode _mode, const std::string& _cachedir, const std::string& _logdir,
                              const char* _nameprefix, int _cache_days, const char* _pub_key, bool _is_compress) {
...
#endif
    appender_open(_mode, _logdir.c_str(), _nameprefix, _pub_key, _is_compress);
