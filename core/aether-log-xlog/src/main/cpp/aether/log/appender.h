...
void appender_open(TAppenderMode _mode, const char* _dir, const char* _nameprefix, const char* _pub_key, bool _is_compress = true);
void appender_open_with_cache(TAppenderMode _mode, const std::string& _cachedir, const std::string& _logdir,
                              const char* _nameprefix, int _cache_days, const char* _pub_key, bool _is_compress = true);
void appender_flush();
...
/*
 * Set custom header information to be written to log file
 *
 * @param _msg    Custom header message (can be NULL to clear)
