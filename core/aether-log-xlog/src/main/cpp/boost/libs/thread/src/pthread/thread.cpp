...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost
{
    namespace detail
    {
        ...
        {
            aether_boost::detail::thread_exit_function_base* func;
            thread_exit_callback_node* next;

            thread_exit_callback_node(aether_boost::detail::thread_exit_function_base* func_,
        ...
        namespace
        {
          ...
          aether_boost::once_flag current_thread_tls_init_flag;
#else
            aether_boost::once_flag current_thread_tls_init_flag=BOOST_ONCE_INIT;
#endif
            ...
            extern "C"
            {
                ...
                {
                    //aether_boost::detail::thread_data_base* thread_info=static_cast<aether_boost::detail::thread_data_base*>(data);
                    aether_boost::detail::thread_data_ptr thread_info = static_cast<aether_boost::detail::thread_data_base*>(data)->shared_from_this();
            ...
            struct  delete_current_thread_tls_key_on_dlclose_t
            {
                ...
                ~delete_current_thread_tls_key_on_dlclose_t()
                {
                    const aether_boost::once_flag uninitialized = BOOST_ONCE_INIT;
                    if (memcmp(&current_thread_tls_init_flag, &uninitialized, sizeof(aether_boost::once_flag)))
                    {
        ...
        aether_boost::detail::thread_data_base* get_current_thread_data()
        {
            aether_boost::call_once(current_thread_tls_init_flag,create_current_thread_tls_key);
            return (aether_boost::detail::thread_data_base*)pthread_getspecific(current_thread_tls_key);
        }
    ...
    namespace
    {
        extern "C"
        {
            ...
            {
                //aether_boost::detail::thread_data_ptr thread_info = static_cast<aether_boost::detail::thread_data_base*>(param)->self;
                aether_boost::detail::thread_data_ptr thread_info = static_cast<aether_boost::detail::thread_data_base*>(param)->shared_from_this();
                thread_info->self.reset();
...
//                 }
                ...
                aether_boost::lock_guard<aether_boost::mutex> lock(thread_info->data_mutex);
      ...
      namespace no_interruption_point
      {
        namespace hiden
        {
          void BOOST_THREAD_DECL sleep_for(const timespec& ts)
          {
                ...
                if (aether_boost::detail::timespec_ge(ts, aether_boost::detail::timespec_zero()))
                {
    ...
    #   endif
          ...
          {
                timespec now = aether_boost::detail::timespec_now();
                if (aether_boost::detail::timespec_gt(ts, now))
                {
                  for (int foo=0; foo < 5; ++foo)
                  {

    #   if defined(BOOST_HAS_PTHREAD_DELAY_NP)
                    timespec d = aether_boost::detail::timespec_minus(ts, now);
    ...
    #   endif
                    timespec now2 = aether_boost::detail::timespec_now();
                    if (aether_boost::detail::timespec_ge(now2, ts))
                    {
                      return;
                    }
                  }
      ...
      namespace hiden
      {
        void BOOST_THREAD_DECL sleep_for(const timespec& ts)
        {
            aether_boost::detail::thread_data_base* const thread_info=aether_boost::detail::get_current_thread_data();
            ...
            {
              aether_boost::this_thread::no_interruption_point::hiden::sleep_for(ts);
            }
        }
        ...
        void BOOST_THREAD_DECL sleep_until(const timespec& ts)
        {
            ...
            {
              aether_boost::this_thread::no_interruption_point::hiden::sleep_until(ts);
            }
        }
      } // hiden
    } // this_thread
...
#   endif
    ...
    unsigned thread::physical_concurrency() BOOST_NOEXCEPT
    {
        ...
        try {
            ...
            while ( getline(proc_cpuinfo, line) ) {
                ...
                aether_boost::split(key_val, line, aether_boost::is_any_of(":"));
                ...
                aether_boost::trim(key);
                aether_boost::trim(value);

                if (key == physical_id) {
                    current_core_entry.first = aether_boost::lexical_cast<unsigned>(value);
                    continue;
                }

                if (key == core_id) {
                    current_core_entry.second = aether_boost::lexical_cast<unsigned>(value);
    ...
    void thread::interrupt()
    {
        ...
        if(local_thread_info)
        {
            ...
            {
                aether_boost::pthread::pthread_mutex_scoped_lock internal_lock(local_thread_info->cond_mutex);
    ...
    namespace this_thread
    {
        void interruption_point()
        {
            ...
            aether_boost::detail::thread_data_base* const thread_info=detail::get_current_thread_data();
    ...
    namespace detail
    {
        ...
        void add_new_tss_node(void const* key,
                              aether_boost::shared_ptr<tss_cleanup_function> func,
                              void* tss_data)
        {
            detail::thread_data_base* const current_thread_data(get_or_make_current_thread_data());
