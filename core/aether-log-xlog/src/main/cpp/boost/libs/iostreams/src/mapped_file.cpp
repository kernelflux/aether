...
{
    if (is_open()) {
        aether_boost::throw_exception(BOOST_IOSTREAMS_FAILURE("file already open"));
        return;
    }
    p.normalize();
    open_file(p);
...
{
    if (!is_open()) {
        aether_boost::throw_exception(BOOST_IOSTREAMS_FAILURE("file is closed"));
        return;
    }
...
void mapped_file_impl::open_file(param_type p)
{
    ...
    if (get_size) {
        ...
        if (get_size(handle_, &info)) {
            aether_boost::intmax_t size =
                ( (static_cast<aether_boost::intmax_t>(info.HighPart) << 32) |
                  info.LowPart );
            size_ =
                static_cast<std::size_t>(
                    p.length != max_length ?
                        std::min<aether_boost::intmax_t>(p.length, size) :
                        size
                );
        } else {
        ...
        {
            aether_boost::intmax_t size =
                (static_cast<aether_boost::intmax_t>(hi) << 32) | low;
            size_ =
                static_cast<std::size_t>(
                    p.length != max_length ?
                        std::min<aether_boost::intmax_t>(p.length, size) :
...
void mapped_file_impl::cleanup_and_throw(const char* msg)
{
    ...
    clear(true);
    BOOST_TRY {
    	aether_boost::iostreams::detail::throw_system_failure(msg);
    } BOOST_CATCH(...) {
    } BOOST_CATCH_END
}
...
{
    if (mode && flags)
        aether_boost::throw_exception(BOOST_IOSTREAMS_FAILURE(
    ...
    if (offset < 0)
        aether_boost::throw_exception(BOOST_IOSTREAMS_FAILURE("invalid offset"));
    if (new_file_size < 0)
        aether_boost::throw_exception(
