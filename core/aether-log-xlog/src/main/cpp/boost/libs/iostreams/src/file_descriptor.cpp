...
void file_descriptor_impl::open(const detail::path& p, BOOST_IOS::openmode mode)
{
         ...
         (BOOST_IOS::in | BOOST_IOS::out) )
    {
        if (mode & BOOST_IOS::app)
            aether_boost::throw_exception(BOOST_IOSTREAMS_FAILURE("bad open mode"));
    ...
    if (fd == -1) {
        aether_boost::throw_exception(system_failure("failed opening file"));
    } else {
        handle_ = fd;
        flags_ = close_always;
    }
    ...
    (stream_offset off, BOOST_IOS::seekdir way)
{
    ...
    {
        aether_boost::throw_exception(system_failure("failed seeking"));
    } else {
    ...
    {
        aether_boost::throw_exception(BOOST_IOSTREAMS_FAILURE("bad offset"));
    }
