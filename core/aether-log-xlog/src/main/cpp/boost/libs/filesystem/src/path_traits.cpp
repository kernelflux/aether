...
namespace pt = aether_boost::filesystem::path_traits;
namespace fs = aether_boost::filesystem;
namespace bs = aether_boost::system;
                   ...
                   const pt::codecvt_type & cvt)
  {
           ...
           to, to_end, to_next)) != std::codecvt_base::ok)
    {
      ...
      BOOST_FILESYSTEM_THROW(bs::system_error(res, fs::codecvt_error_category(),
        "aether_boost::filesystem::path codecvt to wstring"));
    }
...
//--------------------------------------------------------------------------------------//
                   ...
                   const pt::codecvt_type & cvt)
  {
           ...
           to, to_end, to_next)) != std::codecvt_base::ok)
    {
      //std::cout << " result is " << static_cast<int>(res) << std::endl;
      BOOST_FILESYSTEM_THROW(bs::system_error(res, fs::codecvt_error_category(),
        "aether_boost::filesystem::path codecvt to string"));
    }
    target.append(to, to_next); 
  }
...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost { namespace filesystem { namespace path_traits {
...
//--------------------------------------------------------------------------------------//
                ...
                const codecvt_type & cvt)
  {
    ...
    {
      aether_boost::scoped_array< wchar_t > buf(new wchar_t [buf_size]);
...
//--------------------------------------------------------------------------------------//
                ...
                const codecvt_type & cvt)
  {
    ...
    if (buf_size > default_codecvt_buf_size)
    {
      aether_boost::scoped_array< char > buf(new char [buf_size]);
      convert_aux(from, from_end, buf.get(), buf.get()+buf_size, to, cvt);
    }
    else
    {
...
}}} // namespace aether_boost::filesystem::path_traits
