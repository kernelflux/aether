...
void fail(int err, aether_boost::system::error_code* ec)
{
  if (ec == 0)
    BOOST_FILESYSTEM_THROW( aether_boost::system::system_error(err,
      aether_boost::system::system_category(),
      "aether_boost::filesystem::unique_path"));

  ec->assign(err, aether_boost::system::system_category());
  return;
}
...
void system_crypt_random(void* buf, std::size_t len, aether_boost::system::error_code* ec)
{
# ifdef BOOST_POSIX_API
...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost { namespace filesystem { namespace detail {
