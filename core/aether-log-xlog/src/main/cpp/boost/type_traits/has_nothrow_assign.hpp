...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost {
   ...
   namespace detail
   {
      ...
      template <class T>          struct has_nothrow_assign_imp<T, false, true>{ static const bool value = noexcept(aether_boost::declval<typename add_reference<T>::type>() = aether_boost::declval<typename add_reference<T const>::type>()); };
   ...
   struct has_nothrow_assign : public integral_constant < bool,
      ...
      >::value
#else
      ::aether_boost::has_trivial_assign<T>::value
#endif
#else
