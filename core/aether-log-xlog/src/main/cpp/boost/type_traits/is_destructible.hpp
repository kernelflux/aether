...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename = decltype(aether_boost::declval<T&>().~T())>
   ...
   template <class T> struct is_destructible : public integral_constant<bool, sizeof(detail::is_destructible_imp::test<T>(0)) == sizeof(aether_boost::type_traits::yes_type)>{};
