...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost { namespace iostreams {
                    
//------------------Definitions of constants----------------------------------//

namespace gzip {

using namespace aether_boost::iostreams::zlib;
...
public:
    ...
    std::streamsize write(Sink& snk, const char_type* s, std::streamsize n)
    {
        if (!(flags_ & f_header_done)) {
            ...
            offset_ += aether_boost::iostreams::write(snk, header_.data() + offset_, amt);
...
private:
    ...
    template<typename Sink>
    static void write_long(long n, Sink& next, aether_boost::mpl::true_)
    {
        aether_boost::iostreams::put(next, static_cast<char>(0xFF & n));
        aether_boost::iostreams::put(next, static_cast<char>(0xFF & (n >> 8)));
        aether_boost::iostreams::put(next, static_cast<char>(0xFF & (n >> 16)));
        aether_boost::iostreams::put(next, static_cast<char>(0xFF & (n >> 24)));
    }
    template<typename Sink>
    static void write_long(long n, Sink& next, aether_boost::mpl::false_)
    {
    }
    template<typename Sink>
...
} // End namespace aether_boost::iostreams::detail.
...
public:
        ...
        while(result < n) {
            ...
            if (state_ == s_header) {
                ...
                BOOST_TRY {
                    ...
                    aether_boost::throw_exception(gzip_error(e));
                }
            } else { // state_ == s_footer
                if (footer_.done()) {
                    if (footer_.crc() != this->crc())
                        aether_boost::throw_exception(gzip_error(gzip::bad_crc));
    ...
    std::streamsize read(Source& src, char_type* s, std::streamsize n)
    {
        ...
        while (result < n && state_ != s_done) {
            ...
            if (state_ == s_header) {
                int c = aether_boost::iostreams::get(peek);
                if (traits_type::is_eof(c)) {
                    aether_boost::throw_exception(gzip_error(gzip::bad_header));
                ...
                int c = aether_boost::iostreams::get(peek);
                if (traits_type::is_eof(c)) {
                    aether_boost::throw_exception(gzip_error(gzip::bad_footer));
                } else if (traits_type::would_block(c)) {
                    break;
                }
                footer_.process(c);
...
private:
    ...
    struct peekable_source {
        ...
        std::streamsize read(char* s, std::streamsize n)
        {
            ...
            std::streamsize amt = 
                aether_boost::iostreams::read(src_, s + result, n - result);
            return amt != -1 ? 
                result + amt : 
                result ? result : -1;
        }
        bool putback(char c)
        {
            if (offset_) {
                ...
                aether_boost::throw_exception(
                    aether_boost::iostreams::detail::bad_putback());
            }
            return true;
        }
...
{
    aether_boost::iostreams::back_insert_device<std::string> out(footer_);
