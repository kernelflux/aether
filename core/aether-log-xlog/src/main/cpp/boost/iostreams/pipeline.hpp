...
#define BOOST_IOSTREAMS_PIPABLE(filter, arity) \
    ...
    ::aether_boost::iostreams::pipeline< \
        ::aether_boost::iostreams::detail::pipeline_segment< \
    ...
    > operator|( const filter BOOST_IOSTREAMS_TEMPLATE_ARGS(arity, T)& f, \
                 const Component& c ) \
    { \
        typedef ::aether_boost::iostreams::detail::pipeline_segment< \
                    filter BOOST_IOSTREAMS_TEMPLATE_ARGS(arity, T) \
                > segment; \
        return ::aether_boost::iostreams::pipeline<segment, Component> \
                   (segment(f), c); \
    } \
    /**/
