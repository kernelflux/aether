...
/// \pre n <= std::distance(aether_boost::begin(rng), aether_boost::end(rng))
template< class ForwardRange, class Size, class Value >
inline ForwardRange& fill_n(ForwardRange& rng, Size n, const Value& val)
{
    BOOST_RANGE_CONCEPT_ASSERT(( ForwardRangeConcept<ForwardRange> ));
    BOOST_ASSERT( static_cast<Size>(std::distance(aether_boost::begin(rng), aether_boost::end(rng))) >= n );
    std::fill_n(aether_boost::begin(rng), n, val);
    return rng;
}
