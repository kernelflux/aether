...
inline BOOST_DEDUCED_TYPENAME aether_boost::range_iterator<ForwardRange>::type
remove_if(ForwardRange& rng, UnaryPredicate pred)
{
    BOOST_RANGE_CONCEPT_ASSERT(( ForwardRangeConcept<ForwardRange> ));
    return std::remove_if(aether_boost::begin(rng), aether_boost::end(rng), pred);
}

/// \overload
template< class ForwardRange, class UnaryPredicate >
inline BOOST_DEDUCED_TYPENAME aether_boost::range_iterator<const ForwardRange>::type
...
remove_if(ForwardRange& rng, UnaryPredicate pred)
{
    ...
    return range_return<ForwardRange,re>::pack(
        std::remove_if(aether_boost::begin(rng), aether_boost::end(rng), pred),
        rng);
}
