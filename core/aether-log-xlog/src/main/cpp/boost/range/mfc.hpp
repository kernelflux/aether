...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost { namespace range_detail_microsoft {
    ...
    public:
        ...
        explicit mfc_ptr_array_iterator(ArrayT& arr, INT_PTR index) :
            super_t(index), m_parr(aether_boost::addressof(arr))
        { }
    ...
    public:
        ...
        explicit mfc_map_iterator(MapT const& map, POSITION pos) :
            m_pmap(aether_boost::addressof(map)), m_posNext(pos)
        {
            increment();
        }
    ...
    public:
        ...
        explicit mfc_cpair_map_iterator(MapT& map, PairT *pp) :
            m_pmap(aether_boost::addressof(map)), m_pp(pp)
        { }
    ...
    struct customization< ::CTypedPtrArray<BaseClass, PtrType> >
    {
        ...
        Iterator begin(X& x)
        {
            return Iterator(aether_boost::begin<BaseClass>(x), fun<X>());
        }

        template< class Iterator, class X >
        Iterator end(X& x)
        {
            return Iterator(aether_boost::end<BaseClass>(x), fun<X>());
        }
    };
...
BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TYPE(
    aether_boost::range_detail_microsoft::using_type_as_tag,
