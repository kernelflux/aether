...
public:
  ...
  static IntType pow(IntType a, aether_boost::uintmax_t exponent)
  {
      IntType result = 1;
      while(exponent != 0) {
          if(exponent % 2 == 1) {
...
private:
  ...
  static IntType mult_general(IntType a, IntType b)
  {
    ...
    if(::aether_boost::uintmax_t(modulus) <=
        (::std::numeric_limits< ::aether_boost::uintmax_t>::max)() / modulus)
    {
      return static_cast<IntType>(aether_boost::uintmax_t(a) * b % modulus);
    } else {
