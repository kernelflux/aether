...
public:
    ...
    static void mod_pow_x(aether_boost::uintmax_t exponent, const digit_t * mod, std::size_t mod_bits, digit_t * out)
    {
        const std::size_t bits = std::numeric_limits<digit_t>::digits;
        ...
        aether_boost::uintmax_t i = std::numeric_limits<aether_boost::uintmax_t>::digits - 1;
        while(((aether_boost::uintmax_t(1) << i) & exponent) == 0) {
            --i;
        }
        out[0] = 2;
        std::fill_n(out + 1, n - 1, digit_t(0));
        ...
        while(i--) {
            sqr(out, n);
            m(out, 2 * mod_bits - 1);
            if((aether_boost::uintmax_t(1) << i) & exponent) {
                shift_left(out, n, 1);
...
public:
    ...
    friend polynomial mod_pow_x(aether_boost::uintmax_t exponent, polynomial mod);
private:
...
inline polynomial mod_pow_x(aether_boost::uintmax_t exponent, polynomial mod)
{
    polynomial result;
    mod.normalize();
