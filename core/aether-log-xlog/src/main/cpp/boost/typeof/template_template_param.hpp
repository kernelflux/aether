...
#define BOOST_TYPEOF_TEMPLATE_PARAM_ENCODE(This, n)\
   typedef typename aether_boost::type_of::encode_template<BOOST_PP_CAT(V, n),\
...
#define BOOST_TYPEOF_TEMPLATE_PARAM_DECODE(This, n)\
   typedef aether_boost::type_of::decode_template< BOOST_PP_CAT(iter, n) > BOOST_PP_CAT(d, n);\
...
#define BOOST_TYPEOF_REGISTER_TYPE_FOR_TEMPLATE_TEMPLATE(Name,Params,ID)\
    ...
    struct encode_template_impl<V,Name<\
        ...
        P)> >\
        : aether_boost::type_of::push_back<V, aether_boost::mpl::size_t<ID> >\
    {\
    };\
    template<class Iter> struct decode_template_impl<aether_boost::mpl::size_t<ID>, Iter>\
    {\
