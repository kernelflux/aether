...
#define BOOST_TYPEOF_modifier_support(ID, Fun)\
    ...
    {\
        typedef\
            typename aether_boost::type_of::encode_type<\
            typename aether_boost::type_of::push_back<\
            V\
            , aether_boost::mpl::size_t<ID> >::type\
            , T>::type\
            type;\
    };\
    template<class Iter> struct decode_type_impl<aether_boost::mpl::size_t<ID>, Iter>\
    {\
        typedef aether_boost::type_of::decode_type<Iter> d1;\
...
#define BOOST_TYPEOF_array_support(ID, Qualifier)\
    ...
    {\
        typedef\
            ...
            V\
            , aether_boost::mpl::size_t<ID> >::type\
            , aether_boost::mpl::size_t<N> >::type\
            , T>::type\
        type;\
    };\
    template<class Iter>\
    struct decode_type_impl<aether_boost::mpl::size_t<ID>, Iter>\
    {\
        enum{n = Iter::type::value};\
        typedef aether_boost::type_of::decode_type<typename Iter::next> d;\
