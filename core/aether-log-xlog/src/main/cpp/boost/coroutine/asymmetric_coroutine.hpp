...
# endif
    ...
    {
        push_coroutine tmp( aether_boost::move( other) );
        swap( tmp);
        return * this;
    }
...
# ifdef BOOST_MSVC
        ...
        impl_( 0)
    {
        ...
        impl_ = new ( sp) object_t(
                aether_boost::forward< coroutine_fn >( fn), attrs, detail::preallocated( sp, size, stack_ctx), stack_alloc);
        BOOST_ASSERT( impl_);
        impl_->pull();
    }
...
# endif
        ...
        impl_( 0)
    {
        ...
        // placement new for internal coroutine
        impl_ = new ( sp) object_t(
                aether_boost::forward< Fn >( fn), attrs, detail::preallocated( sp, size, stack_ctx), stack_alloc);
        BOOST_ASSERT( impl_);
        impl_->pull();
    }
    ...
    {
        pull_coroutine tmp( aether_boost::move( other) );
        swap( tmp);
        return * this;
    }
