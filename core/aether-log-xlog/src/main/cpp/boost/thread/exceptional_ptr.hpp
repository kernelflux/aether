...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost
{
  struct exceptional_ptr {
    ...
    template <class E>
    explicit exceptional_ptr(BOOST_FWD_REF(E) ex) : ptr_(aether_boost::copy_exception(aether_boost::forward<E>(ex))) {}
  };

  template <class E>
  inline exceptional_ptr make_exceptional(BOOST_FWD_REF(E) ex) {
    return exceptional_ptr(aether_boost::forward<E>(ex));
  }
