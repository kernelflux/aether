...
#ifndef BOOST_NO_SFINAE
    template<typename T>
    typename enable_if<boost::is_convertible<T&,aether_boost::detail::thread_move_t<T> >, aether_boost::detail::thread_move_t<T> >::type move(T& t)
    {
        return aether_boost::detail::thread_move_t<T>(t);
    }
#endif

    template<typename T>
    aether_boost::detail::thread_move_t<T> move(aether_boost::detail::thread_move_t<T> t)
    {
        return t;
    }
...
#define BOOST_THREAD_RV_REF(TYPE) aether_boost::detail::thread_move_t< TYPE >
#define BOOST_THREAD_RV_REF_BEG aether_boost::detail::thread_move_t<
...
//#define BOOST_THREAD_MAKE_RV_REF(RVALUE) aether_boost::detail::make_rv_ref(RVALUE)
#endif
...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost
{
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
    template <class Tp>
    struct remove_reference : aether_boost::remove_reference<Tp> {};
    template <class Tp>
    struct  decay : aether_boost::decay<Tp> {};
#else
  template <class Tp>
  ...
  {
  private:
    typedef typename aether_boost::move_detail::remove_rvalue_reference<Tp>::type Up0;
    typedef typename aether_boost::remove_reference<Up0>::type Up;
  public:
      ...
      decay_copy(T&& t)
      {
          return aether_boost::forward<T>(t);
      }
#else
  template <class T>
  typename decay<T>::type
