...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost
{
  ...
  {
    inline aether_boost::recursive_mutex& terminal_mutex()
    {
      static aether_boost::recursive_mutex mtx;
      return mtx;
    }
...
#define BOOST_THREAD_LOG \
  { \
    aether_boost::lock_guard<boost::recursive_mutex> _lk_(aether_boost::thread_detail::terminal_mutex()); \
    std::cout << aether_boost::this_thread::get_id() << " - "<<__FILE__<<"["<<__LINE__<<"] " <<std::dec
#else
...
#define BOOST_THREAD_LOG if (true) {} else aether_boost::thread_detail::dummy_stream
#define BOOST_THREAD_END_LOG aether_boost::thread_detail::dummy_stream
