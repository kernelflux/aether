...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost
{
    ...
    public:
        ...
        bool is_this_thread_in()
        {
            ...
            aether_boost::shared_lock<shared_mutex> guard(m);
            for(std::list<thread*>::iterator it=threads.begin(),end=threads.end();
                it!=end;
                ++it)
            {
        ...
        {
            aether_boost::lock_guard<shared_mutex> guard(m);
            aether_boost::csbl::unique_ptr<thread> new_thread(new thread(threadfunc));
