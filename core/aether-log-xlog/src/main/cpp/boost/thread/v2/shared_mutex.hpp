...
public:
    ...
    template <class Rep, class Period>
        bool try_lock_for(const aether_boost::chrono::duration<Rep, Period>& rel_time);
    template <class Clock, class Duration>
        bool
        try_lock_until(
                      const aether_boost::chrono::time_point<Clock, Duration>& abs_time);
    void unlock();
    ...
    template <class Rep, class Period>
        bool
        try_lock_shared_for(const aether_boost::chrono::duration<Rep, Period>& rel_time);
    template <class Clock, class Duration>
        bool
...
public:
    ...
    template <class Rep, class Period>
        ...
        try_lock_upgrade_for(
                            const aether_boost::chrono::duration<Rep, Period>& rel_time);
    template <class Clock, class Duration>
        bool
...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost {
  namespace thread_v2 {
    ...
    public:
      ...
      bool try_lock_for(const aether_boost::chrono::duration<Rep, Period>& rel_time)
      {
        return try_lock_until(aether_boost::chrono::steady_clock::now() + rel_time);
      }
      template <class Clock, class Duration>
      bool
      try_lock_until(
      ...
      try_lock_shared_for(const aether_boost::chrono::duration<Rep, Period>& rel_time)
      {
        return try_lock_shared_until(aether_boost::chrono::steady_clock::now() +
            rel_time);
      }
      template <class Clock, class Duration>
      bool
    ...
    shared_mutex::try_lock_until(
        const aether_boost::chrono::time_point<Clock, Duration>& abs_time)
    {
      aether_boost::unique_lock<mutex_t> lk(mut_);
      if (state_ & write_entered_)
      {
        while (true)
        {
          aether_boost::cv_status status = gate1_.wait_until(lk, abs_time);
          if ((state_ & write_entered_) == 0)
            break;
          if (status == aether_boost::cv_status::timeout)
            return false;
        }
      }
      ...
      if (state_ & n_readers_)
      {
        while (true)
        {
          aether_boost::cv_status status = gate2_.wait_until(lk, abs_time);
    ...
    class upgrade_mutex
    {
      typedef aether_boost::mutex              mutex_t;
      typedef aether_boost::condition_variable cond_t;
      typedef unsigned                count_t;
    ...
    public:
      ...
      try_lock_upgrade_for(
          const aether_boost::chrono::duration<Rep, Period>& rel_time)
      {
        return try_lock_upgrade_until(aether_boost::chrono::steady_clock::now() +
            rel_time);
      }
      template <class Clock, class Duration>
      bool
          ...
          const aether_boost::chrono::duration<Rep, Period>& rel_time)
      {
        return try_unlock_shared_and_lock_until(
            aether_boost::chrono::steady_clock::now() + rel_time);
      }
      template <class Clock, class Duration>
      bool
      try_unlock_shared_and_lock_until(
    ...
    shared_mutex::~shared_mutex()
    {
      aether_boost::lock_guard<mutex_t> _(mut_);
    }
