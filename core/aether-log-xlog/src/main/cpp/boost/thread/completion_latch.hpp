...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost
{
  ...
  private:
    ...
    struct around_wait
    {
      ...
      aether_boost::unique_lock<boost::mutex> &lk_;
      around_wait(completion_latch &that, aether_boost::unique_lock<boost::mutex> &lk)
      : that_(that), lk_(lk)
      {
  ...
  public:
    ...
    count_(count),
    funct_(aether_boost::move(funct)),
    waiters_(0),
    leavers_(0)
    {
    }
    ...
    void wait()
    {
      aether_boost::unique_lock<boost::mutex> lk(mutex_);
    ...
    {
      aether_boost::lock_guard<boost::mutex> lk(mutex_);
      //BOOST_ASSERT(count_ == 0);
      count_ = count;
    }
