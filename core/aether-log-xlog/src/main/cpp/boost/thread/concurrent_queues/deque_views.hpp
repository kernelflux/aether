...
namespace concurrent
{
  ...
  public:
    ...
    void push(BOOST_THREAD_RV_REF(value_type) x) { queue->push_back(aether_boost::move(x)); }
    queue_op_status try_push(BOOST_THREAD_RV_REF(value_type) x) { return queue->try_push_back(aether_boost::move(x)); }
    queue_op_status nonblocking_push(BOOST_THREAD_RV_REF(value_type) x) { return queue->nonblocking_push_back(aether_boost::move(x)); }
    queue_op_status wait_push(BOOST_THREAD_RV_REF(value_type) x) { return queue->wait_push_back(aether_boost::move(x)); }
  };
