...
namespace detail
{
  ...
  class queue_adaptor_copyable_only :
    public aether_boost::queue_base<typename Queue::value_type, typename Queue::size_type>
  {
      Queue queue;
  public:
  ...
  public:
    ...
    void push(BOOST_THREAD_RV_REF(value_type) x) { queue.push(aether_boost::move(x)); }
    queue_op_status try_push(BOOST_THREAD_RV_REF(value_type) x) { return queue.try_push(aether_boost::move(x)); }
    queue_op_status nonblocking_push(BOOST_THREAD_RV_REF(value_type) x) { return queue.nonblocking_push(aether_boost::move(x)); }
    queue_op_status wait_push(BOOST_THREAD_RV_REF(value_type) x) { return queue.wait_push(aether_boost::move(x)); }
  };
