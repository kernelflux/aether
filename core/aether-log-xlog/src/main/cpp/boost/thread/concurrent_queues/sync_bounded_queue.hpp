...
namespace concurrent
{
  ...
  private:
    ...
    {
      elem = aether_boost::move(data_[out_]);
      out_ = inc(out_);
      notify_not_full_if_needed(lk);
    }
    inline value_type pull(unique_lock<mutex>& lk)
    {
      value_type elem = aether_boost::move(data_[out_]);
      out_ = inc(out_);
      notify_not_full_if_needed(lk);
      return aether_boost::move(elem);
    }
    inline aether_boost::shared_ptr<value_type> ptr_pull(unique_lock<mutex>& lk)
    {
      shared_ptr<value_type> res = make_shared<value_type>(aether_boost::move(data_[out_]));
      out_ = inc(out_);
      notify_not_full_if_needed(lk);
      return res;
    }
    ...
    {
      data_[in_] = aether_boost::move(elem);
      set_in(in_p_1, lk);
    }
  };
...
//      iterator_t first = aether_boost::begin(range);
//      iterator_t end = aether_boost::end(range);
//      size_type in = 0;
...
//  }
  ...
  template <typename ValueType>
  aether_boost::shared_ptr<ValueType> sync_bounded_queue<ValueType>::try_pull()
  {
      unique_lock<mutex> lk(mtx_);
      return try_pull(lk);
  }
...
//  }
  ...
  template <typename ValueType>
  aether_boost::shared_ptr<ValueType> sync_bounded_queue<ValueType>::ptr_pull()
  {
      unique_lock<mutex> lk(mtx_);
      wait_until_not_empty(lk);
      return ptr_pull(lk);
  ...
  bool sync_bounded_queue<ValueType>::try_push(BOOST_THREAD_RV_REF(ValueType) elem, unique_lock<mutex>& lk)
  {
    ...
    push_at(aether_boost::move(elem), in_p_1, lk);
    return true;
  }
  ...
  {
      unique_lock<mutex> lk(mtx_);
      return try_push(aether_boost::move(elem), lk);
  }
#endif
  ...
  {
      unique_lock<mutex> lk(mtx_);
      return try_push_back(aether_boost::move(elem), lk);
  }
  ...
  {
    if (closed(lk)) return queue_op_status::closed;
    push_at(aether_boost::move(elem), wait_until_not_full(lk), lk);
    return queue_op_status::success;
  }
  template <typename ValueType>
  ...
  {
    sbq.push_back(aether_boost::move(elem));
    return sbq;
  }
