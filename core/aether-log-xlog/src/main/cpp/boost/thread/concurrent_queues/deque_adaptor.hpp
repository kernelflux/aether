...
namespace detail
{
  ...
  class deque_adaptor_copyable_only :
    public aether_boost::deque_base<typename Queue::value_type, typename Queue::size_type>
  {
      Queue queue;
  public:
  ...
  public:
    ...
    void push_back(BOOST_THREAD_RV_REF(value_type) x) { queue.push_back(aether_boost::move(x)); }
    queue_op_status try_push_back(BOOST_THREAD_RV_REF(value_type) x) { return queue.try_push_back(aether_boost::move(x)); }
    queue_op_status nonblocking_push_back(BOOST_THREAD_RV_REF(value_type) x) { return queue.nonblocking_push_back(aether_boost::move(x)); }
    queue_op_status wait_push_back(BOOST_THREAD_RV_REF(value_type) x) { return queue.wait_push_back(aether_boost::move(x)); }
  };
