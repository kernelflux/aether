...
namespace detail
{
  ...
  struct scheduled_type
  {
    ...
    scheduled_type(BOOST_THREAD_RV_REF(T) pdata, time_point tp) : data(aether_boost::move(pdata)), time(tp) {}
    ...
    scheduled_type(BOOST_THREAD_RV_REF(scheduled_type) other) : data(aether_boost::move(other.data)), time(other.time) {}
    scheduled_type& operator=(BOOST_THREAD_RV_REF(scheduled_type) other) {
      data = aether_boost::move(other.data);
      time = other.time;
      return *this;
    }
  ...
  {
    super::push(stype(aether_boost::move(elem),tp));
  }
  ...
  {
    push(aether_boost::move(elem), clock::now() + dura);
  }
  ...
  {
    return super::try_push(stype(aether_boost::move(elem), tp));
  }
  ...
  {
    return try_push(aether_boost::move(elem), clock::now() + dura);
  }
  ...
  T sync_timed_queue<T, Clock>::pull(unique_lock<mutex>&)
  {
    ...
    return aether_boost::move(super::data_.pull().data);
#else
  ...
  {
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
    elem = aether_boost::move(super::data_.pull().data);
#else
