...
// aether_boost::thread::future requires exception handling
// due to aether_boost::exception::exception_ptr dependency
...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost
{
  ...
  shared_ptr<T const> static_shared_from_this(T const* that)
  {
    ...
    namespace detail
    {
        ...
        struct relocker
        {
            aether_boost::unique_lock<boost::mutex>& lock_;

            relocker(aether_boost::unique_lock<boost::mutex>& lk):
                lock_(lk)
            {
                lock_.unlock();
            }
        ...
        struct shared_state_base : enable_shared_from_this<shared_state_base>
        {
            ...
            aether_boost::exception_ptr exception;
            bool done;
            bool is_valid_;
            bool is_deferred_;
            bool is_constructed;
            launch policy_;
            mutable aether_boost::mutex mutex;
            aether_boost::condition_variable waiters;
            waiter_list external_waiters;
            aether_boost::function<void()> callback;
            ...
            void set_executor_policy(executor_ptr_type aex, aether_boost::lock_guard<boost::mutex>&)
            {
              set_executor();
              ex = aex;
            }
            void set_executor_policy(executor_ptr_type aex, aether_boost::unique_lock<boost::mutex>&)
            {
              set_executor();
              ex = aex;
            }

            bool valid(aether_boost::unique_lock<boost::mutex>&) { return is_valid_; }
            bool valid() {
              aether_boost::unique_lock<boost::mutex> lk(this->mutex);
              return valid(lk);
            }
            void invalidate(aether_boost::unique_lock<boost::mutex>&) { is_valid_ = false; }
            void invalidate() {
              aether_boost::unique_lock<boost::mutex> lk(this->mutex);
              invalidate(lk);
            }
            void validate(aether_boost::unique_lock<boost::mutex>&) { is_valid_ = true; }
            void validate() {
              aether_boost::unique_lock<boost::mutex> lk(this->mutex);
              validate(lk);
            }
...
#endif
            notify_when_ready_handle notify_when_ready(aether_boost::condition_variable_any& cv)
            {
                aether_boost::unique_lock<boost::mutex> lock(this->mutex);
                do_callback(lock);
            ...
            {
                aether_boost::lock_guard<boost::mutex> lock(this->mutex);
                external_waiters.erase(it);
            }

#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
            void do_continuation(aether_boost::unique_lock<boost::mutex>& lock)
            {
...
#else
            void do_continuation(aether_boost::unique_lock<boost::mutex>&)
            {
            }
#endif
#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
            virtual void set_continuation_ptr(continuation_ptr_type continuation, aether_boost::unique_lock<boost::mutex>& lock)
            {
...
#endif
            void mark_finished_internal(aether_boost::unique_lock<boost::mutex>& lock)
            {
                done=true;
                waiters.notify_all();
            ...
            void do_callback(aether_boost::unique_lock<boost::mutex>& lock)
            {
                if(callback && !done)
                {
                    aether_boost::function<void()> local_callback=callback;
            ...
            void wait_internal(aether_boost::unique_lock<boost::mutex> &lk, bool rethrow=true)
            {
              do_callback(lk);
              if (is_deferred_)
              {
              ...
              {
                  aether_boost::rethrow_exception(exception);
              }
            }

            virtual void wait(aether_boost::unique_lock<boost::mutex>& lock, bool rethrow=true)
            {
            ...
            bool timed_wait_until(aether_boost::system_time const& target_time)
            {
                aether_boost::unique_lock<boost::mutex> lock(this->mutex);
                if (is_deferred_)
                    return false;
...
#endif
            void mark_exceptional_finish_internal(aether_boost::exception_ptr const& e, aether_boost::unique_lock<boost::mutex>& lock)
            {
                exception=e;
                mark_finished_internal(lock);
            }
            ...
            {
                aether_boost::unique_lock<boost::mutex> lock(this->mutex);
                mark_exceptional_finish_internal(aether_boost::current_exception(), lock);
            }
            ...
            launch launch_policy(aether_boost::unique_lock<boost::mutex>&) const
            {
                return policy_;
            }

            future_state::state get_state(aether_boost::unique_lock<boost::mutex>&) const
            {
                if(!done)
                {
            ...
            {
                aether_boost::lock_guard<boost::mutex> guard(this->mutex);
                if(!done)
                {
                    return future_state::waiting;
                }
            ...
            {
                aether_boost::lock_guard<boost::mutex> lock(this->mutex);
                callback=boost::bind(f,aether_boost::ref(*u));
            }

            virtual void execute(aether_boost::unique_lock<boost::mutex>&) {}
        ...
        {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
              typedef aether_boost::optional<T> storage_type;
#else
              typedef aether_boost::csbl::unique_ptr<T> storage_type;
#endif
            ...
            void mark_finished_with_result_internal(source_reference_type result_, aether_boost::unique_lock<boost::mutex>& lock)
            {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
                result = result_;
#else
            ...
            void mark_finished_with_result_internal(rvalue_source_type result_, aether_boost::unique_lock<boost::mutex>& lock)
            {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
                result = aether_boost::move(result_);
#elif ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
                result.reset(new T(aether_boost::move(result_)));
#else
            ...
            template <class ...Args>
            void mark_finished_with_result_internal(aether_boost::unique_lock<boost::mutex>& lock, BOOST_THREAD_FWD_REF(Args)... args)
            {
#if defined BOOST_THREAD_FUTURE_USES_OPTIONAL
                result.emplace(aether_boost::forward<Args>(args)...);
#else
                result.reset(new T(aether_boost::forward<Args>(args)...));
#endif
                this->mark_finished_internal(lock);
            }
#endif
            ...
            void mark_finished_with_result(rvalue_source_type result_)
            {
                ...
                mark_finished_with_result_internal(aether_boost::move(result_), lock);
#else
                mark_finished_with_result_internal(static_cast<rvalue_source_type>(result_), lock);
#endif
            }

            storage_type& get_storage(aether_boost::unique_lock<boost::mutex>& lk)
            {
                wait_internal(lk);
                return result;
            }
            virtual move_dest_type get(aether_boost::unique_lock<boost::mutex>& lk)
            {
                return aether_boost::move(*get_storage(lk));
            }
            move_dest_type get()
            {
                aether_boost::unique_lock<boost::mutex> lk(this->mutex);
            ...
            virtual shared_future_get_result_type get_sh(aether_boost::unique_lock<boost::mutex>& lk)
            {
            ...
            detail::shared_state_base
        {
            ...
            virtual T& get(aether_boost::unique_lock<boost::mutex>& lock)
            {
                wait_internal(lock);
                return *result;
            }
            ...
            virtual T& get_sh(aether_boost::unique_lock<boost::mutex>& lock)
            {
                wait_internal(lock);
                return *result;
            }
            ...
            detail::shared_state_base
        {
            ...
            void mark_finished_with_result_internal(aether_boost::unique_lock<boost::mutex>& lock)
            {
                mark_finished_internal(lock);
            }
            ...
            virtual void get(aether_boost::unique_lock<boost::mutex>& lock)
            {
            ...
            virtual void get_sh(aether_boost::unique_lock<boost::mutex>& lock)
            {
                this->wait_internal(lock);
            }
            void get_sh()
        ...
        public:
          ...
          virtual void wait(aether_boost::unique_lock<boost::mutex>& lk, bool rethrow)
          {
#ifdef BOOST_THREAD_FUTURE_BLOCKING
              {
                relocker rlk(lk);
        ...
        struct future_async_shared_state: future_async_shared_state_base<Rp>
        {
          ...
          {
#ifdef BOOST_THREAD_FUTURE_BLOCKING
            this->thr_ = thread(&future_async_shared_state::run, static_shared_from_this(this), aether_boost::forward<Fp>(f));
#else
            thread(&future_async_shared_state::run, static_shared_from_this(this), aether_boost::forward<Fp>(f)).detach();
#endif
          }
        ...
        struct future_async_shared_state<void, Fp>: public future_async_shared_state_base<void>
        {
          ...
          {
#ifdef BOOST_THREAD_FUTURE_BLOCKING
            this->thr_ = thread(&future_async_shared_state::run, static_shared_from_this(this), aether_boost::move(f));
#else
            thread(&future_async_shared_state::run, static_shared_from_this(this), aether_boost::move(f)).detach();
#endif
          }
        ...
        public:
          ...
          : func_(aether_boost::move(f))
          {
            this->set_deferred();
          }

          virtual void execute(aether_boost::unique_lock<boost::mutex>& lck) {
            try
            {
              ...
              Rp res = local_fuct();
              relock.lock();
              this->mark_finished_with_result_internal(aether_boost::move(res), lck);
            }
            catch (...)
            {
        ...
        private:
            ...
            struct registered_waiter
            {
                aether_boost::shared_ptr<detail::shared_state_base> future_;
                detail::shared_state_base::notify_when_ready_handle handle;
                count_type index;

                registered_waiter(aether_boost::shared_ptr<detail::shared_state_base> const& a_future,
            ...
            struct all_futures_lock
            {
                   ...
                   aether_boost::scoped_array<boost::unique_lock<boost::mutex> > locks;

                all_futures_lock(std::vector<registered_waiter>& futures):
                    count(futures.size()),locks(new aether_boost::unique_lock<boost::mutex>[count])
                {
                    for(count_type_portable i=0;i<count;++i)
                    {
                        locks[i]=BOOST_THREAD_MAKE_RV_REF(aether_boost::unique_lock<boost::mutex>(futures[i].future_->mutex));
                    }
                }

                void lock()
                {
                    aether_boost::lock(locks.get(),locks.get()+count);
                }
            ...
            aether_boost::condition_variable_any cv;
...
//    typename aether_boost::disable_if<is_future_type<Iterator>,Iterator>::type wait_for_any(Iterator begin,Iterator end)
//    {
//        if(begin==end)
//            return end;
//
...
//            waiter.add(*current);
//        }
//        return aether_boost::next(begin,waiter.wait());
//    }

#ifdef BOOST_NO_CXX11_VARIADIC_TEMPLATES
    template<typename F1,typename F2>
    typename aether_boost::enable_if<is_future_type<F1>,typename detail::future_waiter::count_type>::type wait_for_any(F1& f1,F2& f2)
    {
        detail::future_waiter waiter;
        waiter.add(f1);
        waiter.add(f2);
...
#else
    template<typename F1, typename... Fs>
    typename aether_boost::enable_if<is_future_type<F1>, typename detail::future_waiter::count_type>::type
    ...
    namespace detail
    {
      ...
      public:
        ...
        typedef aether_boost::shared_ptr<detail::shared_state<R> > future_ptr;
      ...
      public:
        ...
        state get_state(aether_boost::unique_lock<boost::mutex>& lk) const
        {
            if(!future_)
            {
        ...
        bool is_ready(aether_boost::unique_lock<boost::mutex>& lk) const
        {
        ...
        launch launch_policy(aether_boost::unique_lock<boost::mutex>& lk) const
        {
        ...
        {
          if ( future_ ) {
            aether_boost::unique_lock<boost::mutex> lk(this->future_->mutex);
        ...
        void wait() const
        {
            if(!future_)
            {
                aether_boost::throw_exception(future_uninitialized());
            }
            future_->wait(false);
        }
        ...
        {
            return timed_wait_until(aether_boost::get_system_time()+rel_time);
        }

        bool timed_wait_until(aether_boost::system_time const& abs_time) const
        {
            if(!future_)
            {
                aether_boost::throw_exception(future_uninitialized());
    ...
    namespace detail
    {
        ...
        BOOST_THREAD_FUTURE<Rp>
        make_future_async_continuation_shared_state(aether_boost::unique_lock<boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_future_sync_continuation_shared_state(aether_boost::unique_lock<boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_future_deferred_continuation_shared_state(aether_boost::unique_lock<boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_deferred_continuation_shared_state(aether_boost::unique_lock<boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_async_continuation_shared_state(aether_boost::unique_lock<boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_sync_continuation_shared_state(aether_boost::unique_lock<boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);


  #ifdef BOOST_THREAD_PROVIDES_EXECUTORS
        template<typename Ex, typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_future_executor_continuation_shared_state(Ex& ex, aether_boost::unique_lock<boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename Ex, typename F, typename Rp, typename Fp>
        BOOST_THREAD_FUTURE<Rp>
        make_shared_future_executor_continuation_shared_state(Ex& ex, aether_boost::unique_lock<boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);
        ...
        make_future_unwrap_shared_state(aether_boost::unique_lock<boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f);
#endif
    }
#if defined(BOOST_THREAD_PROVIDES_FUTURE_WHEN_ALL_WHEN_ANY)
      template< typename InputIterator>
      typename aether_boost::disable_if<is_future_type<InputIterator>,
    ...
    private:
        ...
        detail::make_future_async_continuation_shared_state(aether_boost::unique_lock<boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_sync_continuation_shared_state(aether_boost::unique_lock<boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template <class F, class Rp, class Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_deferred_continuation_shared_state(aether_boost::unique_lock<boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_deferred_continuation_shared_state(aether_boost::unique_lock<boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_async_continuation_shared_state(aether_boost::unique_lock<boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_sync_continuation_shared_state(aether_boost::unique_lock<boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);

  #ifdef BOOST_THREAD_PROVIDES_EXECUTORS
        template<typename Ex, typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_executor_continuation_shared_state(Ex& ex, aether_boost::unique_lock<boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c);

        template<typename Ex, typename F, typename Rp, typename Fp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_shared_future_executor_continuation_shared_state(Ex& ex, aether_boost::unique_lock<boost::mutex> &lock, F f, BOOST_THREAD_FWD_REF(Fp) c);
        ...
        template <class F, class Rp>
        friend BOOST_THREAD_FUTURE<Rp>
        detail::make_future_unwrap_shared_state(aether_boost::unique_lock<boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f);
#endif
#if defined(BOOST_THREAD_PROVIDES_FUTURE_WHEN_ALL_WHEN_ANY)
      template< typename InputIterator>
      friend typename aether_boost::disable_if<is_future_type<InputIterator>,
        BOOST_THREAD_FUTURE<csbl::vector<typename InputIterator::value_type>  >
      >::type
      when_all(InputIterator first, InputIterator last);
    ...
    public:
        ...
        base_type(aether_boost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))))
        {
        }
        ...
        {
            this->base_type::operator=(aether_boost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))));
            return *this;
        }
        ...
        typename aether_boost::disable_if< is_void<R2>, move_dest_type>::type
...
#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
        template<typename F>
        inline BOOST_THREAD_FUTURE<typename aether_boost::result_of<F(BOOST_THREAD_FUTURE)>::type>
        ...
        template <typename R2>
        inline typename aether_boost::disable_if< is_void<R2>, BOOST_THREAD_FUTURE<R> >::type
      ...
      when_any(BOOST_THREAD_FWD_REF(T0) f, BOOST_THREAD_FWD_REF(T) ... futures);
        ...
        public:
            ...
            move_dest_type get_or(BOOST_THREAD_RV_REF(R) v) // EXTENSION
            {
                ...
                else return aether_boost::move(v);
            }
    ...
    public:
        ...
        {
            base_type::operator=(aether_boost::move(static_cast<base_type&>(BOOST_THREAD_RV(other))));
            return *this;
        }
        ...
        template <typename R2>
        typename aether_boost::disable_if< is_void<R2>, typename detail::shared_state<R>::shared_future_get_result_type>::type
...
#if defined BOOST_THREAD_PROVIDES_FUTURE_CONTINUATION
        template<typename F>
        inline BOOST_THREAD_FUTURE<typename aether_boost::result_of<F(shared_future)>::type>
...
#include <boost/detail/atomic_redef_macros.hpp>
    ...
    public:
        ...
        promise(aether_boost::allocator_arg_t, Allocator a)
        {
        ...
        ~promise()
        {
            if(future_)
            {
                aether_boost::unique_lock<boost::mutex> lock(future_->mutex);

                if(!future_->done && !future_->is_constructed)
                {
                    future_->mark_exceptional_finish_internal(aether_boost::copy_exception(broken_promise()), lock);
                }
            }
        }
        ...
        void set_executor(executor_ptr_type aex)
        {
          ...
          if (future_.get()==0)
          {
              aether_boost::throw_exception(promise_moved());
          }
          aether_boost::lock_guard<boost::mutex> lk(future_->mutex);
        ...
        BOOST_THREAD_FUTURE<R> get_future()
        {
            ...
            if (future_obtained)
            {
                aether_boost::throw_exception(future_already_retrieved());
            }
...
#if defined  BOOST_NO_CXX11_RVALUE_REFERENCES
        template <class TR>
        typename aether_boost::enable_if_c<is_copy_constructible<TR>::value && is_same<R, TR>::value, void>::type set_value(TR const &  r)
        {
            lazy_init();
            aether_boost::unique_lock<boost::mutex> lock(future_->mutex);
            if(future_->done)
            {
                aether_boost::throw_exception(promise_already_satisfied());
            }
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
            future_->mark_finished_with_result_internal(aether_boost::move(r), lock);
#else
        ...
        void emplace(BOOST_THREAD_FWD_REF(Args) ...args)
        {
            ...
            future_->mark_finished_with_result_internal(lock, aether_boost::forward<Args>(args)...);
        }

#endif

        void set_exception(aether_boost::exception_ptr p)
        {
            lazy_init();
            aether_boost::unique_lock<boost::mutex> lock(future_->mutex);
            if(future_->done)
        ...
        {
          set_exception(aether_boost::copy_exception(ex));
        }
        // setting the result with deferred notification
#if defined  BOOST_NO_CXX11_RVALUE_REFERENCES
        template <class TR>
        typename aether_boost::enable_if_c<is_copy_constructible<TR>::value && is_same<R, TR>::value, void>::type set_value_at_thread_exit(TR const& r)
        {
          if (future_.get()==0)
          {
              aether_boost::throw_exception(promise_moved());
        ...
        void set_value_at_thread_exit(BOOST_THREAD_RV_REF(R) r)
        {
          ...
          future_->set_value_at_thread_exit(aether_boost::move(r));
        }
        void set_exception_at_thread_exit(exception_ptr e)
        {
          if (future_.get()==0)
        ...
        {
          set_exception_at_thread_exit(aether_boost::copy_exception(ex));
        }
...
#include <boost/detail/atomic_redef_macros.hpp>
    ...
    class promise<void>
    {
        typedef aether_boost::shared_ptr<detail::shared_state<void> > future_ptr;
...
#if defined BOOST_THREAD_PROVIDES_FUTURE_CTOR_ALLOCATORS
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost { namespace container {
...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost
{
    ...
    namespace detail
    {
      ...
      struct task_base_shared_state:
            ...
            detail::shared_state<R>
        {
            ...
            {
                {
                    aether_boost::lock_guard<boost::mutex> lk(this->mutex);
                    if(started)
                    {
                        aether_boost::throw_exception(task_already_started());
                    }
                    started=true;
                }
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
                do_run(aether_boost::move(args)...);
#else
                do_run();
#endif
            }
...
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK && defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
                do_apply(aether_boost::move(args)...);
#else
                do_apply();
#endif
            }
            ...
            void owner_destroyed()
            {
                ...
                {
                    started=true;
                    this->mark_exceptional_finish_internal(aether_boost::copy_exception(aether_boost::broken_promise()), lk);
                }
            }
        };
        ...
        public:
            ...
            task_shared_state(BOOST_THREAD_RV_REF(F) f_):
              f(aether_boost::move(f_))
            {}

            F callable()
            {
              return aether_boost::move(f);
            }
            ...
            void do_apply(BOOST_THREAD_RV_REF(ArgTypes) ... args)
            {
                try
                {
                    this->set_value_at_thread_exit(f(aether_boost::move(args)...));
                }
#else
            void do_apply()
            {
            ...
            void do_run(BOOST_THREAD_RV_REF(ArgTypes) ... args)
            {
                try
                {
                    this->mark_finished_with_result(f(aether_boost::move(args)...));
                }
#else
            void do_run()
            {
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
                  R res((f()));
                  this->mark_finished_with_result(aether_boost::move(res));
#else
           ...
           task_base_shared_state<R>
            ...
            public:
                ...
                void do_apply()
                {
                    ...
                    {
                        R r((f()));
                        this->set_value_at_thread_exit(aether_boost::move(r));
                    }
#endif
                    catch(...)
                    {
    ...
    {
      typedef aether_boost::shared_ptr<detail::task_base_shared_state<R(ArgTypes...)> > task_ptr;
      aether_boost::shared_ptr<detail::task_base_shared_state<R(ArgTypes...)> > task;
  #else
    template<typename R>
    class packaged_task<R()>
    {
      typedef aether_boost::shared_ptr<detail::task_base_shared_state<R()> > task_ptr;
      aether_boost::shared_ptr<detail::task_base_shared_state<R()> > task;
  #endif
#else
    template<typename R>
    class packaged_task
    ...
    public:
        ...
        explicit packaged_task(R(*f)(), BOOST_THREAD_FWD_REF(ArgTypes)... args)
        {
            ...
            task= task_ptr(new task_shared_state_type(f, aether_boost::move(args)...));
            future_obtained=false;
        }
  #else
        ...
        explicit packaged_task(BOOST_THREAD_FWD_REF(F) f
            , typename aether_boost::disable_if<is_same<typename decay<F>::type, packaged_task>, dummy* >::type=0
            )
        {
          typedef typename decay<F>::type FR;
            ...
            typedef detail::task_shared_state<FR,R> task_shared_state_type;
#endif
            task = task_ptr(new task_shared_state_type(aether_boost::forward<F>(f)));
            future_obtained = false;
...
#if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
            typedef detail::task_shared_state<F,R(ArgTypes...)> task_shared_state_type;
            task = task_ptr(new task_shared_state_type(aether_boost::move(f)));
#else
            typedef detail::task_shared_state<F,R()> task_shared_state_type;
            task = task_ptr(new task_shared_state_type(aether_boost::move(f)));
#endif
...
#if defined(BOOST_THREAD_RVALUE_REFERENCES_DONT_MATCH_FUNTION_PTR)
        template <class Allocator>
        packaged_task(aether_boost::allocator_arg_t, Allocator a, R(*f)())
        {
          typedef R(*FR)();
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
        ...
        packaged_task(aether_boost::allocator_arg_t, Allocator a, BOOST_THREAD_FWD_REF(F) f)
        {
          typedef typename decay<F>::type FR;
          ...
          task = task_ptr(::new(a2.allocate(1)) task_shared_state_type(aether_boost::forward<F>(f)), D(a2, 1) );
          future_obtained = false;
        }
#else // ! defined BOOST_NO_CXX11_RVALUE_REFERENCES
        template <class F, class Allocator>
        packaged_task(aether_boost::allocator_arg_t, Allocator a, const F& f)
        {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          typedef detail::task_shared_state<F,R(ArgTypes...)> task_shared_state_type;
        ...
        template <class F, class Allocator>
        packaged_task(aether_boost::allocator_arg_t, Allocator a, BOOST_THREAD_RV_REF(F) f)
        {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
  #if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
          typedef detail::task_shared_state<F,R(ArgTypes...)> task_shared_state_type;
          ...
          task = task_ptr(::new(a2.allocate(1)) task_shared_state_type(aether_boost::move(f)), D(a2, 1) );
          future_obtained = false;
        }
...
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
            packaged_task temp(aether_boost::move(other));
#else
        ...
        {
          if (!valid())
              aether_boost::throw_exception(task_moved());
          aether_boost::lock_guard<boost::mutex> lk(task->mutex);
          task->set_executor_policy(aex, lk);
        }
#endif
        void reset() {
            if (!valid())
              aether_boost::throw_exception(future_error(system::make_error_code(future_errc::no_state)));
        ...
        void operator()(ArgTypes... args) {
            ...
            task->run(aether_boost::move(args)...);
        }
        void make_ready_at_thread_exit(ArgTypes... args) {
          ...
          task->apply(aether_boost::move(args)...);
        }
#else
        void operator()() {
            if(!task) {
        ...
        void make_ready_at_thread_exit() {
          ...
          if (task->has_value()) aether_boost::throw_exception(promise_already_satisfied());
          task->apply();
        }
#endif
        template<typename F>
...
namespace detail
{
  ...
  make_future_deferred_shared_state(BOOST_THREAD_FWD_REF(Fp) f) {
    shared_ptr<future_deferred_shared_state<Rp, Fp> >
        h(new future_deferred_shared_state<Rp, Fp>(aether_boost::forward<Fp>(f)));
  ...
  make_future_async_shared_state(BOOST_THREAD_FWD_REF(Fp) f) {
    ...
    h->init(aether_boost::forward<Fp>(f));
    return BOOST_THREAD_FUTURE<Rp>(h);
  }
}
  ...
  async(launch policy, R(*f)(BOOST_THREAD_FWD_REF(ArgTypes)...), BOOST_THREAD_FWD_REF(ArgTypes)... args) {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF(aether_boost::detail::make_future_async_shared_state<Rp>(
              BF(
                  f
                  , thread_detail::decay_copy(aether_boost::forward<ArgTypes>(args))...
              )
          ));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      return BOOST_THREAD_MAKE_RV_REF(aether_boost::detail::make_future_deferred_shared_state<Rp>(
              BF(
                  f
                  , thread_detail::decay_copy(aether_boost::forward<ArgTypes>(args))...
              )
  ...
  async(launch policy, R(*f)()) {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      ...
      ret.set_async();
      aether_boost::thread( aether_boost::move(pt) ).detach();
      return ::boost::move(ret);
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      std::terminate();
      //BOOST_THREAD_FUTURE<R> ret;
  ...
  template <class F, class ...ArgTypes>
  BOOST_THREAD_FUTURE<typename aether_boost::result_of<typename decay<F>::type(
  ...
  async(launch policy, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(ArgTypes)... args) {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF(aether_boost::detail::make_future_async_shared_state<Rp>(
              BF(
                  thread_detail::decay_copy(aether_boost::forward<F>(f))
                , thread_detail::decay_copy(aether_boost::forward<ArgTypes>(args))...
              )
          ));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
  ...
  template <class F>
  BOOST_THREAD_FUTURE<typename aether_boost::result_of<typename decay<F>::type()>::type>
  async(launch policy, BOOST_THREAD_FWD_REF(F) f) {
    typedef typename aether_boost::result_of<typename decay<F>::type()>::type R;
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
    typedef packaged_task<R()> packaged_task_type;
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      packaged_task_type pt( aether_boost::forward<F>(f) );
      ...
      //BOOST_THREAD_FUTURE<R> ret;
      //return ::boost::move(ret);
      //          return aether_boost::detail::make_future_deferred_shared_state<Rp>(
      //              BF(
      //                  thread_detail::decay_copy(aether_boost::forward<F>(f))
      //              )
      //          );
...
namespace detail {
    ...
    public:
      ...
      : that(st), f_(aether_boost::move(f))
      {};
      ...
      : that(x.that), f_(aether_boost::move(x.f_))
      {
        x.that.reset();
      }
...
namespace detail {
    ...
    public:
      ...
      void init(Executor& ex, BOOST_THREAD_FWD_REF(Fp) f)
      {
        ...
        shared_state_nullary_task<Rp,Cont> t(this->shared_from_this(), aether_boost::forward<Fp>(f));
        ex.submit(aether_boost::move(t));
      }
    ...
    make_future_executor_shared_state(Executor& ex, BOOST_THREAD_FWD_REF(Fp) f) {
      ...
      h->init(ex, aether_boost::forward<Fp>(f));
      return BOOST_THREAD_FUTURE<Rp>(h);
    }
...
//#if ! defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
  ...
  async(Executor& ex, R(*f)(BOOST_THREAD_FWD_REF(ArgTypes)...), BOOST_THREAD_FWD_REF(ArgTypes)... args) {
    ...
    return BOOST_THREAD_MAKE_RV_REF(aether_boost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            f
            , thread_detail::decay_copy(aether_boost::forward<ArgTypes>(args))...
        )
  ...
  async(Executor& ex, R(*f)(BOOST_THREAD_FWD_REF(A1)), BOOST_THREAD_FWD_REF(A1) a1) {
    ...
    return BOOST_THREAD_MAKE_RV_REF(aether_boost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            f
            , thread_detail::decay_copy(aether_boost::forward<A1>(a1))
        )
    ));
  }
#endif // defined BOOST_THREAD_RVALUE_REFERENCES_DONT_MATCH_FUNTION_PTR
  ...
  async(Executor& ex, BOOST_THREAD_FWD_REF(F) f)  {
    ...
    return aether_boost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            thread_detail::decay_copy(aether_boost::forward<F>(f))
        )
    );
  ...
  async(Executor& ex, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(A1) a1, BOOST_THREAD_FWD_REF(A2) a2) {
    ...
    return BOOST_THREAD_MAKE_RV_REF(aether_boost::detail::make_future_executor_shared_state<Rp>(ex,
        BF(
            thread_detail::decay_copy(aether_boost::forward<F>(f))
          , thread_detail::decay_copy(aether_boost::forward<A1>(a1))
          , thread_detail::decay_copy(aether_boost::forward<A2>(a2))
        )
    ));
  }
  ...
  async(R(*f)(BOOST_THREAD_FWD_REF(ArgTypes)...), BOOST_THREAD_FWD_REF(ArgTypes)... args) {
    return BOOST_THREAD_MAKE_RV_REF(async(launch(launch::any), f, aether_boost::forward<ArgTypes>(args)...));
  }
  #else
  template <class R>
  BOOST_THREAD_FUTURE<R>
  ...
  async(BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(ArgTypes)... args) {
      return BOOST_THREAD_MAKE_RV_REF(async(launch(launch::any), aether_boost::forward<F>(f), aether_boost::forward<ArgTypes>(args)...));
  }
#else
  template <class F>
  BOOST_THREAD_FUTURE<typename aether_boost::result_of<F()>::type>
  async(BOOST_THREAD_FWD_REF(F) f) {
      return BOOST_THREAD_MAKE_RV_REF(async(launch(launch::any), aether_boost::forward<F>(f)));
  }
#endif
  ...
  BOOST_THREAD_FUTURE<typename decay<T>::type> make_future(BOOST_THREAD_FWD_REF(T) value) {
    ...
    p.set_value(aether_boost::forward<future_value_type>(value));
  ...
  BOOST_THREAD_FUTURE<typename detail::deduced_type<T>::type> make_ready_future(BOOST_THREAD_FWD_REF(T) value) {
    ...
    promise<future_value_type> p;
    p.set_value(aether_boost::forward<T>(value));
    return BOOST_THREAD_MAKE_RV_REF(p.get_future());
  }
  ...
  BOOST_THREAD_FUTURE<T> make_exceptional_future(E ex) {
    promise<T> p;
    p.set_exception(aether_boost::copy_exception(ex));
    return BOOST_THREAD_MAKE_RV_REF(p.get_future());
  }
  ...
  BOOST_THREAD_FUTURE<T> make_exceptional_future() {
    promise<T> p;
    p.set_exception(aether_boost::current_exception());
    return BOOST_THREAD_MAKE_RV_REF(p.get_future());
  }
  template <typename T>
...
namespace detail
{
  ...
  public:
    ...
    : parent(aether_boost::move(f)),
      continuation(aether_boost::move(c))
    {
    }

    void init(aether_boost::unique_lock<boost::mutex> &lock)
    {
    ...
    void call() {
      try {
        this->mark_finished_with_result(this->continuation(aether_boost::move(this->parent)));
      } catch(...) {
        this->mark_exceptional_finish();
      }
  ...
  public:
    ...
    {
      try {
        this->continuation(aether_boost::move(this->parent));
        this->mark_finished_with_result();
      } catch(...) {
        this->mark_exceptional_finish();
      }
  ...
  public:
    ...
    : base_type(aether_boost::move(f), aether_boost::forward<Fp>(c))
    {    }
...
namespace detail {
  ...
  public:
    ...
    template <class Ex>
    void init(aether_boost::unique_lock<boost::mutex> &lk, Ex& ex)
    {
    ...
    void launch_continuation() {
      ...
      this->get_executor()->submit(aether_boost::move(fct));
    }
  ...
  public:
    ...
    virtual void execute(aether_boost::unique_lock<boost::mutex>& lk) {
      this->parent.wait();
      this->call(lk);
    }
  ...
  BOOST_THREAD_FUTURE<Rp>
  make_future_deferred_continuation_shared_state(
      aether_boost::unique_lock<boost::mutex> &lock,
      BOOST_THREAD_RV_REF(F) f, BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    shared_ptr<future_deferred_continuation_shared_state<F, Rp, Cont> >
        h(new future_deferred_continuation_shared_state<F, Rp, Cont>(aether_boost::move(f), aether_boost::forward<Fp>(c)));
    h->init(lock);
    return BOOST_THREAD_FUTURE<Rp>(h);
  }
  ...
  BOOST_THREAD_FUTURE<Rp>
  make_future_async_continuation_shared_state(
      aether_boost::unique_lock<boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f,
      BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    shared_ptr<future_async_continuation_shared_state<F,Rp, Cont> >
        h(new future_async_continuation_shared_state<F,Rp, Cont>(aether_boost::move(f), aether_boost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_future_sync_continuation_shared_state(
    ...
    shared_ptr<future_sync_continuation_shared_state<F,Rp, Cont> >
        h(new future_sync_continuation_shared_state<F,Rp, Cont>(aether_boost::move(f), aether_boost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_future_executor_continuation_shared_state(Ex& ex,
    ...
    shared_ptr<future_executor_continuation_shared_state<F,Rp, Cont> >
        h(new future_executor_continuation_shared_state<F,Rp, Cont>(aether_boost::move(f), aether_boost::forward<Fp>(c)));
    h->init(lock, ex);
  ...
  make_shared_future_deferred_continuation_shared_state(
    ...
    shared_ptr<shared_future_deferred_continuation_shared_state<F, Rp, Cont> >
        h(new shared_future_deferred_continuation_shared_state<F, Rp, Cont>(f, aether_boost::forward<Fp>(c)));
    h->init(lock);
  ...
  BOOST_THREAD_FUTURE<Rp>
  make_shared_future_async_continuation_shared_state(
      aether_boost::unique_lock<boost::mutex> &lock, F f,
      BOOST_THREAD_FWD_REF(Fp) c) {
    typedef typename decay<Fp>::type Cont;
    shared_ptr<shared_future_async_continuation_shared_state<F,Rp, Cont> >
        h(new shared_future_async_continuation_shared_state<F,Rp, Cont>(f, aether_boost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_shared_future_sync_continuation_shared_state(
    ...
    shared_ptr<shared_future_sync_continuation_shared_state<F,Rp, Cont> >
        h(new shared_future_sync_continuation_shared_state<F,Rp, Cont>(f, aether_boost::forward<Fp>(c)));
    h->init(lock);
  ...
  make_shared_future_executor_continuation_shared_state(Ex& ex,
    ...
    shared_ptr<shared_future_executor_continuation_shared_state<F, Rp, Cont> >
        h(new shared_future_executor_continuation_shared_state<F, Rp, Cont>(f, aether_boost::forward<Fp>(c)));
    h->init(lock, ex);
  ...
  template <typename R>
  template <typename F>
  inline BOOST_THREAD_FUTURE<typename aether_boost::result_of<F(BOOST_THREAD_FUTURE<R>)>::type>
  BOOST_THREAD_FUTURE<R>::then(launch policy, BOOST_THREAD_FWD_REF(F) func) {
    typedef typename aether_boost::result_of<F(BOOST_THREAD_FUTURE<R>)>::type future_type;
    ...
    aether_boost::unique_lock<boost::mutex> lock(sentinel->mutex);

    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF((aether_boost::detail::make_future_async_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type>(
                  lock, aether_boost::move(*this), aether_boost::forward<F>(func)
              )));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      return BOOST_THREAD_MAKE_RV_REF((aether_boost::detail::make_future_deferred_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type>(
                  lock, aether_boost::move(*this), aether_boost::forward<F>(func)
              )));
#ifdef BOOST_THREAD_PROVIDES_EXECUTORS
      ...
      return BOOST_THREAD_MAKE_RV_REF((aether_boost::detail::make_future_executor_continuation_shared_state<Ex, BOOST_THREAD_FUTURE<R>, future_type>(ex,
                    lock, aether_boost::move(*this), aether_boost::forward<F>(func)
                )));
#endif
  ...
  BOOST_THREAD_FUTURE<R>::then(BOOST_THREAD_FWD_REF(F) func)  {
    ...
    return this->then(this->launch_policy(), aether_boost::forward<F>(func));
#else
    typedef typename aether_boost::result_of<F(BOOST_THREAD_FUTURE<R>)>::type future_type;
    BOOST_THREAD_ASSERT_PRECONDITION(this->future_!=0, future_uninitialized());
  ...
  template <typename R2>
  template <typename F>
  inline BOOST_THREAD_FUTURE<typename aether_boost::result_of<F(BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2> >)>::type>
  BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2> >::then(launch policy, BOOST_THREAD_FWD_REF(F) func) {
    ...
    if (underlying_cast<int>(policy) & int(launch::async)) {
      ...
      return BOOST_THREAD_MAKE_RV_REF((aether_boost::detail::make_future_sync_continuation_shared_state<BOOST_THREAD_FUTURE<R>, future_type>(
                  lock, aether_boost::move(*this), aether_boost::forward<F>(func)
              )));
#ifdef BOOST_THREAD_PROVIDES_EXECUTORS
    } else if (underlying_cast<int>(policy) & int(launch::executor)) {
  ...
  template <typename R>
  template <typename F>
  inline BOOST_THREAD_FUTURE<typename aether_boost::result_of<F(shared_future<R>)>::type>
  shared_future<R>::then(launch policy, BOOST_THREAD_FWD_REF(F) func)  const
  {
    typedef typename aether_boost::result_of<F(shared_future<R>)>::type future_type;
    BOOST_THREAD_ASSERT_PRECONDITION(this->future_!=0, future_uninitialized());

    aether_boost::unique_lock<boost::mutex> lock(this->future_->mutex);
    if (underlying_cast<int>(policy) & int(launch::async)) {
      return BOOST_THREAD_MAKE_RV_REF((aether_boost::detail::make_shared_future_async_continuation_shared_state<shared_future<R>, future_type>(
                  lock, *this, aether_boost::forward<F>(func)
              )));
    } else if (underlying_cast<int>(policy) & int(launch::deferred)) {
      return BOOST_THREAD_MAKE_RV_REF((aether_boost::detail::make_shared_future_deferred_continuation_shared_state<shared_future<R>, future_type>(
                  lock, *this, aether_boost::forward<F>(func)
              )));
    } else if (underlying_cast<int>(policy) & int(launch::sync)) {
      return BOOST_THREAD_MAKE_RV_REF((aether_boost::detail::make_shared_future_sync_continuation_shared_state<shared_future<R>, future_type>(
                  lock, *this, aether_boost::forward<F>(func)
              )));
#ifdef BOOST_THREAD_PROVIDES_EXECUTORS
    } else if (underlying_cast<int>(policy) & int(launch::executor)) {
      typedef executor Ex;
      Ex& ex = *(this->future_->get_executor());
      return BOOST_THREAD_MAKE_RV_REF((aether_boost::detail::make_shared_future_executor_continuation_shared_state<Ex, shared_future<R>, future_type>(ex,
                    lock, *this, aether_boost::forward<F>(func)
                )));
#endif
    } else if (underlying_cast<int>(policy) & int(launch::inherit)) {
...
namespace detail
{
  ...
  struct mfallbacker_to
  {
    ...
    : value_(aether_boost::move(v))
    {}

    T operator()(BOOST_THREAD_FUTURE<T> fut) {
      return fut.get_or(aether_boost::move(value_));
    }
  };
  template <typename T>
  ...
  BOOST_THREAD_FUTURE<R>::fallback_to(BOOST_THREAD_RV_REF(R2) v) {
    return then(detail::mfallbacker_to<R>(aether_boost::move(v)));
  }
...
namespace detail
{
    ...
    void launch_continuation()
    {
      ...
      if (! unwrapped.valid() )
      {
        if (wrapped.has_exception()) {
          ...
          {
            lk.unlock();
            aether_boost::unique_lock<boost::mutex> lk2(unwrapped.future_->mutex);
            unwrapped.future_->set_continuation_ptr(this->shared_from_this(), lk2);
          } else {
            this->mark_exceptional_finish_internal(aether_boost::copy_exception(future_uninitialized()), lk);
          }
        }
      } else {
  ...
  make_future_unwrap_shared_state(aether_boost::unique_lock<boost::mutex> &lock, BOOST_THREAD_RV_REF(F) f) {
    shared_ptr<future_unwrap_shared_state<F, Rp> >
        h(new future_unwrap_shared_state<F, Rp>(aether_boost::move(f)));
  ...
  BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2> >::unwrap()
  {
    ...
    return aether_boost::detail::make_future_unwrap_shared_state<BOOST_THREAD_FUTURE<BOOST_THREAD_FUTURE<R2> >, R2>(lock, aether_boost::move(*this));
  }
#endif
...
namespace detail
{
  ...
  struct future_when_all_vector_shared_state: future_async_shared_state_base<csbl::vector<F> >
  {
    ...
    static void run(shared_ptr<boost::detail::shared_state_base> that_) {
      ...
      try {
        aether_boost::wait_for_all(that->vec_.begin(), that->vec_.end());
        that->mark_finished_with_result(aether_boost::move(that->vec_));
      } catch(...) {
        that->mark_exceptional_finish();
      }
    }
  ...
  public:
    ...
    future_when_all_vector_shared_state(values_tag, BOOST_THREAD_FWD_REF(T0) f, BOOST_THREAD_FWD_REF(T) ... futures) {
      vec_.push_back(aether_boost::forward<T0>(f));
      typename alias_t<char[]>::type{
          ( //first part of magic unpacker
          vec_.push_back(aether_boost::forward<T>(futures)),'0'
          )..., '0'
  ...
  struct future_when_any_vector_shared_state: future_async_shared_state_base<csbl::vector<F> >
  {
    ...
    static void run(shared_ptr<boost::detail::shared_state_base> that_)
    {
      ...
      try {
        aether_boost::wait_for_any(that->vec_.begin(), that->vec_.end());
        that->mark_finished_with_result(aether_boost::move(that->vec_));
      } catch(...) {
        that->mark_exceptional_finish();
      }
  ...
  public:
    ...
    future_when_any_vector_shared_state(values_tag,
      ...
      typename alias_t<char[]>::type{
          ( //first part of magic unpacker
          vec_.push_back(aether_boost::forward<T>(futures))
          ,'0'
          )...,
          '0'
      }; //second part of magic unpacker
  ...
  struct wait_for_all_fctr {
    template <class ...T>
    void operator()(T&&... v) {
      aether_boost::wait_for_all(aether_boost::forward<T>(v)...);
    }
  };
  ...
  struct wait_for_any_fctr {
    template <class ...T>
    void operator()(T&&... v) {
      aether_boost::wait_for_any(aether_boost::forward<T>(v)...);
    }
  };
  ...
  struct future_when_all_tuple_shared_state: future_async_shared_state_base<Tuple>
  {
    ...
    static void run(shared_ptr<boost::detail::shared_state_base> that_) {
      ...
      try {
        // TODO make use of apply(that->tup_, aether_boost::detail::wait_for_all_fctor());
        that->wait_for_all(Index());

        that->mark_finished_with_result(aether_boost::move(that->tup_));
      } catch(...) {
        that->mark_exceptional_finish();
      }
    }
  ...
  public:
    ...
    future_when_all_tuple_shared_state(values_tag, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(Fs) ... futures) :
      tup_(aether_boost::csbl::make_tuple(aether_boost::forward<F>(f), aether_boost::forward<Fs>(futures)...))
    {
    }
  ...
  when_all(BOOST_THREAD_FWD_REF(T0) f, BOOST_THREAD_FWD_REF(T) ... futures) {
    ...
    shared_ptr<factory_type>
        h(new factory_type(detail::values_tag_value, aether_boost::forward<T0>(f), aether_boost::forward<T>(futures)...));
    h->init();
    return BOOST_THREAD_FUTURE<container_type>(h);
  }
#endif
