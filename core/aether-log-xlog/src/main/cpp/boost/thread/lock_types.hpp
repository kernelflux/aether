...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost
{
  ...
  public:
    ...
    void lock()
    {
      if (m == 0)
      {
        aether_boost::throw_exception(
            aether_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost unique_lock has no mutex"));
      }
      if (owns_lock())
      {
        aether_boost::throw_exception(
            aether_boost::lock_error(static_cast<int>(system::errc::resource_deadlock_would_occur), "boost unique_lock owns already the mutex"));
      }
      m->lock();
      is_locked = true;
    }
    ...
    bool timed_lock(TimeDuration const& relative_time)
    {
      if(m==0)
      {
        aether_boost::throw_exception(aether_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost unique_lock has no mutex"));
      }
      if(owns_lock())
      {
        aether_boost::throw_exception(aether_boost::lock_error(static_cast<int>(system::errc::resource_deadlock_would_occur), "boost unique_lock owns already the mutex"));
      }
    ...
    void unlock()
    {
      ...
      {
        aether_boost::throw_exception(
            aether_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost unique_lock doesn't own the mutex"));
      }
      m->unlock();
      is_locked = false;
    }
  ...
  public:
    ...
    void lock()
    {
      if(m==0)
      {
        aether_boost::throw_exception(aether_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost shared_lock has no mutex"));
      }
      if(owns_lock())
      {
        aether_boost::throw_exception(aether_boost::lock_error(static_cast<int>(system::errc::resource_deadlock_would_occur), "boost shared_lock owns already the mutex"));
      }
      m->lock_shared();
      is_locked=true;
    }
...
#if defined BOOST_THREAD_USES_DATETIME
    bool timed_lock(aether_boost::system_time const& target_time)
    {
      if(m==0)
      {
        aether_boost::throw_exception(aether_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost shared_lock has no mutex"));
    ...
    void unlock()
    {
      ...
      if(!owns_lock())
      {
        aether_boost::throw_exception(aether_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost shared_lock doesn't own the mutex"));
      }
      m->unlock_shared();
      is_locked=false;
    }
  ...
  public:
    ...
    void lock()
    {
      ...
      {
        aether_boost::throw_exception(
            aether_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost shared_lock has no mutex"));
      }
      if (owns_lock())
      {
        aether_boost::throw_exception(
            aether_boost::lock_error(static_cast<int>(system::errc::resource_deadlock_would_occur), "boost upgrade_lock owns already the mutex"));
      }
      m->lock_upgrade();
      is_locked = true;
    }
    ...
    void unlock()
    {
      ...
      {
        aether_boost::throw_exception(
            aether_boost::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost upgrade_lock doesn't own the mutex"));
      }
      m->unlock_upgrade();
      is_locked = false;
    }
