...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost
{
  namespace thread_detail
  {
    ...
    struct void_functor_barrier_reseter
    {
      ...
      : size_(size), fct_(aether_boost::move(funct))
      {}
      template <typename F>
      ...
      //size_(BOOST_THREAD_RV(other).size_), fct_(aether_boost::move(BOOST_THREAD_RV(other).fct_))
      {
      }
  ...
  class barrier
  {
    ...
    {
      if (count == 0) aether_boost::throw_exception(
  ...
  public:
    ...
    bool wait()
    {
      aether_boost::unique_lock < aether_boost::mutex > lock(m_mutex);
