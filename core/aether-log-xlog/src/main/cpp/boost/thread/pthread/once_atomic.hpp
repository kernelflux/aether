...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost
{
  ...
  namespace thread_detail
  {
    ...
    typedef aether_boost::atomic<atomic_int_type> atomic_type;
...
#define BOOST_ONCE_INIT aether_boost::once_flag()
...
#define BOOST_THREAD_INVOKE_RET_VOID_CALL
#else
#define BOOST_THREAD_INVOKE_RET_VOID aether_boost::bind
#define BOOST_THREAD_INVOKE_RET_VOID_CALL ()
#endif
    ...
    if (thread_detail::enter_once_region(flag))
    {
      BOOST_TRY
      {
        BOOST_THREAD_INVOKE_RET_VOID(
                        thread_detail::decay_copy(aether_boost::forward<Function>(f)),
  ...
  inline void call_once(once_flag& flag, Function f, T1 p1)
  {
    if (thread_detail::enter_once_region(flag))
    {
      BOOST_TRY
      {
        aether_boost::forward<Function>(f)(aether_boost::forward<T1>(p1));
  ...
  inline void call_once(once_flag& flag, BOOST_THREAD_RV_REF(Function) f, BOOST_THREAD_RV_REF(T1) p1)
  {
    if (thread_detail::enter_once_region(flag))
    {
      BOOST_TRY
      {
        BOOST_THREAD_INVOKE_RET_VOID(
            thread_detail::decay_copy(aether_boost::forward<Function>(f)),
            thread_detail::decay_copy(aether_boost::forward<T1>(p1))
        ) BOOST_THREAD_INVOKE_RET_VOID_CALL;
      }
      BOOST_CATCH (...)
      {
    ...
    if (thread_detail::enter_once_region(flag))
    {
      BOOST_TRY
      {
        BOOST_THREAD_INVOKE_RET_VOID(
            thread_detail::decay_copy(aether_boost::forward<Function>(f)),
            thread_detail::decay_copy(aether_boost::forward<T1>(p1)),
            thread_detail::decay_copy(aether_boost::forward<T1>(p2))
        ) BOOST_THREAD_INVOKE_RET_VOID_CALL;
      }
      BOOST_CATCH (...)
      {
    ...
    if (thread_detail::enter_once_region(flag))
    {
      BOOST_TRY
      {
        BOOST_THREAD_INVOKE_RET_VOID(
            ...
            thread_detail::decay_copy(aether_boost::forward<T1>(p1)),
            thread_detail::decay_copy(aether_boost::forward<T1>(p2)),
            thread_detail::decay_copy(aether_boost::forward<T1>(p3))
        ) BOOST_THREAD_INVOKE_RET_VOID_CALL;
