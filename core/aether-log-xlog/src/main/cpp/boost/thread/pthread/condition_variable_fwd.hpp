  ...
  namespace detail {
    ...
    //private: // used by aether_boost::thread::try_join_until
        ...
        bool do_wait_for(
            ...
            struct timespec const &timeout)
        {
          return do_wait_until(lock, aether_boost::detail::timespec_plus(timeout, aether_boost::detail::timespec_now()));
        }
    ...
    public:
      BOOST_THREAD_NO_COPYABLE(condition_variable)
        condition_variable()
        {
            ...
            if(res)
            {
                aether_boost::throw_exception(thread_resource_error(res, "boost::condition_variable::condition_variable() constructor failed in pthread_mutex_init"));
            }
#endif
...
#endif
                aether_boost::throw_exception(thread_resource_error(res, "boost::condition_variable::condition_variable() constructor failed in detail::monotonic_pthread_cond_init"));
            }
        }
        ~condition_variable()
        {
        ...
        inline bool timed_wait(
            unique_lock<mutex>& m,
            aether_boost::system_time const& abs_time)
        {
        ...
        bool timed_wait(
            unique_lock<mutex>& m,
            aether_boost::system_time const& abs_time,predicate_type pred)
        {
            while (!pred())
            {
