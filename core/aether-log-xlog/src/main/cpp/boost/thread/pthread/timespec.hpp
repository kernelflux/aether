...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost
{
  ...
  {
#if defined BOOST_THREAD_USES_DATETIME
    inline struct timespec to_timespec(aether_boost::system_time const& abs_time)
    {
    ...
    inline timespec to_timespec(aether_boost::intmax_t const& ns)
    {
      aether_boost::intmax_t s = ns / 1000000000l;
      struct timespec ts;
      ts.tv_sec = static_cast<long> (s);
      ts.tv_nsec = static_cast<long> (ns - s * 1000000000l);
      return ts;
    }
    inline aether_boost::intmax_t to_nanoseconds_int_max(timespec const& ts)
    {
