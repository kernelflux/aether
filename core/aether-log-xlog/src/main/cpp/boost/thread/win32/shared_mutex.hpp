...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost
{
    ...
    public:
        ...
        bool timed_lock_shared(aether_boost::system_time const& wait_until)
        {
            for(;;)
            {
                state_data old_state=state;
                for(;;)
                {
                    ...
                    if(new_state.exclusive || new_state.exclusive_waiting_blocked)
                    {
                        ...
                        {
                            aether_boost::throw_exception(aether_boost::lock_error());
                        }
                    }
                    else
                    {
...
#if defined BOOST_THREAD_USES_DATETIME
        bool timed_lock(aether_boost::system_time const& wait_until)
        {
            for(;;)
            {
                state_data old_state=state;
