  ...
  namespace detail
  {
    void handle_task_region_exceptions(exception_list& errors)
    {
      ...
      catch (exception_list const& el)
      {
        ...
        {
          aether_boost::exception_ptr const& e = *it;
          try {
            rethrow_exception(e);
          }
          catch (...)
      ...
      {
        errors.add(aether_boost::current_exception());
      }
    }
    ...
    void wait_all()
    {
      ...
      for (group_type::iterator it = group.begin(); it != group.end(); ++it)
      {
        ...
        if (f.has_exception())
        {
          try
          {
            aether_boost::rethrow_exception(f.get_exception_ptr());
          }
          catch (...)
          {
      ...
      if (exs.size() != 0)
      {
        aether_boost::throw_exception(exs);
      }
    }
protected:
  ...
  public:
    ...
    void run(BOOST_THREAD_FWD_REF(F) f)
    {
#if defined BOOST_THREAD_TASK_REGION_HAS_SHARED_CANCELED
      {
        ...
        if (canceled) {
          aether_boost::throw_exception(task_canceled_exception());
        }
      }
#if defined BOOST_THREAD_PROVIDES_EXECUTORS
