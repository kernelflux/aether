...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost
{
   ...
   * Example:
   *
   *     aether_boost::strict_scoped_thread<> t((aether_boost::thread(F)));
   *
   */
  ...
  public:
    ...
    explicit strict_scoped_thread(BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(Args)... args) :
      t_(aether_boost::forward<F>(f), aether_boost::forward<Args>(args)...) {}
#else
    template <class F>
    ...
    strict_scoped_thread(BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(A1) a1, BOOST_THREAD_FWD_REF(A2) a2, BOOST_THREAD_FWD_REF(A3) a3) :
      t_(aether_boost::forward<F>(f), aether_boost::forward<A1>(a1), aether_boost::forward<A2>(a2), aether_boost::forward<A3>(a3)) {}
#endif
   ...
   * Example:
   *
   *     aether_boost::scoped_thread<> t((aether_boost::thread(F)));
   *     t.interrupt();
   *
   */
  template <class CallableThread = join_if_joinable>
...
//    }
    ...
    t_(aether_boost::move(BOOST_THREAD_RV(x).t_))
    {}
