...
{
template<typename T> aether_boost::shared_ptr<T> shared_from_raw(T *);
template<typename T> aether_boost::weak_ptr<T> weak_from_raw(T *);
...
private:
    ...
    template<typename T> friend aether_boost::shared_ptr<T> shared_from_raw(T *);
    template<typename T> friend aether_boost::weak_ptr<T> weak_from_raw(T *);
    template< class X, class Y > friend inline void detail::sp_enable_shared_from_this( aether_boost::shared_ptr<X> * ppx, Y const * py, aether_boost::enable_shared_from_raw const * pe );
#endif
    ...
    template<class X, class Y> void _internal_accept_owner( shared_ptr<X> * ppx, Y * py ) const
    {
        ...
        else if( shared_this_.use_count() != 0 )
        {
            ...
            detail::esft2_deleter_wrapper * pd = aether_boost::get_deleter<detail::esft2_deleter_wrapper>( shared_this_ );
            BOOST_ASSERT( pd != 0 );
...
{
    BOOST_ASSERT(p != 0);
    return aether_boost::shared_ptr<T>(p->enable_shared_from_raw::shared_from_this(), p);
}
...
{
    BOOST_ASSERT(p != 0);
    aether_boost::weak_ptr<T> result;
