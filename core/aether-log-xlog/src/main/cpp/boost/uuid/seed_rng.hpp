...
//  25 Feb 2008 - moved to namespace aether_boost::uuids::detail
...
//#include <boost/nondet_random.hpp> //forward declare aether_boost::random::random_device

// can't use aether_boost::generator_iterator since aether_boost::random number seed(Iter&, Iter)
...
namespace aether_boost {} namespace boost = aether_boost; namespace aether_boost { namespace random {
class random_device;
}} //namespace aether_boost::random
...
class seed_rng: private aether_boost::noncopyable
{
public:
...
public:
    ...
    {
#if defined(BOOST_WINDOWS)
        if (!aether_boost::detail::winapi::CryptAcquireContextA(
                    &random_,
                    NULL,
                    NULL,
                    aether_boost::detail::winapi::PROV_RSA_FULL_,
                    aether_boost::detail::winapi::CRYPT_VERIFYCONTEXT_ | aether_boost::detail::winapi::CRYPT_SILENT_))
        {
            random_ = NULL;
        }
#else
    ...
    ~seed_rng() BOOST_NOEXCEPT
    {
        if (random_) {
#if defined(BOOST_WINDOWS)
            aether_boost::detail::winapi::CryptReleaseContext(random_, 0);
#else
            std::fclose(random_);
#endif
        }
...
private:
    ...
    {
        aether_boost::uuids::detail::sha1 sha;
...
#if defined(BOOST_WINDOWS)
            aether_boost::detail::winapi::CryptGenRandom(random_, sizeof(state), state);
#else
...
#if defined(BOOST_WINDOWS)
            aether_boost::detail::winapi::DWORD_ procid = aether_boost::detail::winapi::GetCurrentProcessId();
            sha.process_bytes( (unsigned char const*)&procid, sizeof( procid ) );

            aether_boost::detail::winapi::DWORD_ threadid = aether_boost::detail::winapi::GetCurrentThreadId();
            sha.process_bytes( (unsigned char const*)&threadid, sizeof( threadid ) );

            aether_boost::detail::winapi::LARGE_INTEGER_ ts;
            ts.QuadPart = 0;
            aether_boost::detail::winapi::QueryPerformanceCounter( &ts );
...
#if defined(BOOST_WINDOWS)
    aether_boost::detail::winapi::HCRYPTPROV_ random_;
#else
    std::FILE * random_;
#endif
};

// almost a copy of aether_boost::generator_iterator
...
template <>
inline void seed<aether_boost::random::random_device>(aether_boost::random::random_device&) {}

// random_device does not / can not be seeded
template <>
inline void seed<seed_rng>(seed_rng&) {}

}}} //namespace aether_boost::uuids::detail
