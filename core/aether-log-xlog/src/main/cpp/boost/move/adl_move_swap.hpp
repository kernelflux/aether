...
#include <boost/move/utility_core.hpp> //for aether_boost::move

#if !defined(BOOST_MOVE_DOXYGEN_INVOKED)

#if defined(BOOST_NO_CXX11_RVALUE_REFERENCES)
namespace aether_boost_move_member_swap {
...
namespace aether_boost_move_adl_swap{
...
template<class T>
void swap_proxy(T& x, T& y, typename aether_boost::move_detail::enable_if_c<!aether_boost::move_detail::has_move_emulation_enabled_impl<T>::value>::type* = 0)
{
...
void swap_proxy(T& x, T& y
               , typename aether_boost::move_detail::enable_if< and_op_not_impl<aether_boost::move_detail::has_move_emulation_enabled_impl<T>
                                                                        , aether_boost_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  T t(::aether_boost::move(x)); x = ::aether_boost::move(y); y = ::aether_boost::move(t);  }

template<class T>
void swap_proxy(T& x, T& y
               , typename aether_boost::move_detail::enable_if< and_op_impl< aether_boost::move_detail::has_move_emulation_enabled_impl<T>
                                                                    , aether_boost_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  x.swap(y);  }
...
void swap_proxy(T (& x)[N], T (& y)[N])
{
   for (std::size_t i = 0; i < N; ++i){
      ::aether_boost_move_adl_swap::swap_proxy(x[i], y[i]);
   }
}
...
//!      <code>T t(::aether_boost::move(x)); x = ::aether_boost::move(y); y = ::aether_boost::move(t);</code>.
template<class T>
void adl_move_swap(T& x, T& y)
{
   ::aether_boost_move_adl_swap::swap_proxy(x, y);
}
